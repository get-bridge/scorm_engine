RSpec.describe ScormEngine::Api::Endpoints::About do
  ...
  describe "#about_user_count" do
    ...
    it "accepts :before option" do
      subject = ScormEngine::Client.new(tenant: "ScormEngineGemTesting").about_user_count(before: Time.parse("1901-01-1"))
      aggregate_failures do
        expect(subject.success?).to eq true
    ...
    it "accepts :since option" do
      subject = ScormEngine::Client.new(tenant: "ScormEngineGemTesting").about_user_count(since: Time.parse("2031-01-1"))
      aggregate_failures do
        expect(subject.success?).to eq true
        expect(subject.result.total).to eq 0
      end
